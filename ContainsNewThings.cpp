...
//Drone state
struct Drone {
    float pos[3] = {0.0f, 5.0f, 0.0f}; // Initial position
    float velocity[3] = {0.0f, 0.0f, 0.0f};
    float speed = 0.0f;
    float throttle = 0.0f; // Up/down
    float yaw = 0.0f;      // Rotation around Y
    float pitch = 0.0f;    // Forward/backward tilt
...
{
	glutPostRedisplay();
	glutTimerFunc(1000 / 90, refresh, 0);	// Sets the timer to run at 90 FPS
}
...
void updateDroneState(float dt) {
    ...
    const float throttleSpeed = 5.0f;
    const float yawSpeed = 60.0f; // degrees/sec
    const float maxTilt = 30.0f; // degrees

    // Calculate direction from pitch and roll
    float pitchRad = drone.pitch * 3.14159f / 180.0f;
    float rollRad = drone.roll * 3.14159f / 180.0f;
    float yawRad = drone.yaw * 3.14159f / 180.0f;

    // Forward direction (local drone space)
    float dir[3] = {
        sin(yawRad) * cos(pitchRad),
        0.0f,
        cos(yawRad) * cos(pitchRad)
    };
    // Add roll effect (sideways movement)
    dir[0] += sin(rollRad);
    dir[2] += -sin(rollRad);
    ...
    float targetSpeed = maxSpeed * (fabs(drone.pitch) + fabs(drone.roll)) / maxTilt;
    if (targetSpeed > maxSpeed) targetSpeed = maxSpeed;

    // Smooth speed change
    if (targetSpeed > drone.speed)
        drone.speed += accel * dt;
    else
        drone.speed -= drag * dt;
    if (drone.speed < 0.0f) drone.speed = 0.0f;
    if (drone.speed > maxSpeed) drone.speed = maxSpeed;

    // Update velocity
    drone.velocity[0] = dir[0] * drone.speed;
    drone.velocity[2] = dir[2] * drone.speed;
    drone.velocity[1] = drone.throttle * throttleSpeed;
...
void drawDrone(dataMesh &data) {
    ...
    mu.scale(gmu::MODEL, 2.0f, 0.5f, 2.0f); // Squashed cube
...
void renderSim(void) {
	...
	switch (cameraMode) {
		case FOLLOW:
			mu.lookAt(drone.pos[0] + camX, drone.pos[1] + camY, drone.pos[2] + camZ, drone.pos[0], drone.pos[1], drone.pos[2], 0, 1, 0);
			break;
		case TOP_ORTHO:
			mu.lookAt(0, 50, 0, 0, 0, 0, 0, 0, -1);
			mu.loadIdentity(gmu::PROJECTION);
	...
	for (int i = 0; i < 4; ++i) {
		for (int j = 0; j < 4; ++j) {
			...
			data.meshID = objId;
			data.texMode = i % 4;   //0:no texturing; 1:modulate diffuse color with texel color; 2:diffuse color is replaced by texel color; 3: multitexturing
			data.vm = mu.get(gmu::VIEW_MODEL),
			data.pvm = mu.get(gmu::PROJ_VIEW_MODEL);
			data.normal = mu.getNormalMatrix();
			renderer.renderMesh(data);
	...
	mu.pushMatrix(gmu::MODEL);
	mu.translate(gmu::MODEL, - 1.5f, 0.0f, - 1.5f);
	mu.computeDerivedMatrix(gmu::PROJ_VIEW_MODEL);
	mu.computeNormalMatrix3x3();
	data.meshID = 0;
	data.texMode = 3;   //0:no texturing; 1:modulate diffuse color with texel color; 2:diffuse color is replaced by texel color; 3: multitexturing
	data.vm = mu.get(gmu::VIEW_MODEL),
	data.pvm = mu.get(gmu::PROJ_VIEW_MODEL);
	data.normal = mu.getNormalMatrix();
	renderer.renderMesh(data);
...
void processSpecialKeys(int key, int xx, int yy) {
    switch (key) {
        case GLUT_KEY_UP: drone.pitch = -20.0f; break;
        case GLUT_KEY_DOWN: drone.pitch = 20.0f; break;
        case GLUT_KEY_LEFT: drone.roll = -20.0f; break;
        case GLUT_KEY_RIGHT: drone.roll = 20.0f; break;
    }
}
void processSpecialUpKeys(int key, int xx, int yy) {
    switch (key) {
        case GLUT_KEY_UP: drone.pitch = 0.0f; break;
        case GLUT_KEY_DOWN: drone.pitch = 0.0f; break;
        case GLUT_KEY_LEFT: drone.pitch = 0.0f; break;
        case GLUT_KEY_RIGHT: drone.roll = 0.0f; break;
    }
}

void processKeyUp(unsigned char key, int xx, int yy) {
    if (key == 'w' || key == 's') drone.throttle = 0.0f;
}
...
void processKeys(unsigned char key, int xx, int yy)
{
	switch(key) {
		...
		// Change camera mode
		case '1': cameraMode = TOP_ORTHO;
			printf("Camera mode: TOP ORTHO\n");
			break;
		case '2': cameraMode = TOP_PERSPECTIVE;
			printf("Camera mode: TOP PERSPECTIVE\n");
			break;
		case '3': cameraMode = FOLLOW;
			printf("Camera mode: FOLLOW\n");
			break;
		
		// Move drone
		case 'w': drone.throttle = 1.0f; break;
        case 's': drone.throttle = -1.0f; break;
        case 'a': drone.yaw -= 5.0f; break;
        case 'd': drone.yaw += 5.0f; break;
	}
}
